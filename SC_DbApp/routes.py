...
def register_routes(app, db):
    ...
    def admin_dashboard():
        ...
        try:
            ...
            for book in all_books:
                ...
                category_to_books.setdefault(key, []).append(book)
            ...
            # compute category counts (safe)
            category_counts = (
                db.session.query(Book.category, func.count(Book.book_id))
    ...
    def edit_book(book_id):
        ...
        if request.method == 'POST':
            ...
            if isbn:
                existing = Book.query.filter(Book.isbn == isbn, Book.book_id != book.book_id).first()
                if existing:
